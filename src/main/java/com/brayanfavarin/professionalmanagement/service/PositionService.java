...
@Service public class PositionService {
 ...
 @Transactional(readOnly=true) public PageResponse<PositionResponse> list(String search, Pageable pageable){String normalizedSearch=InputNormalizer.optional(search);Page<Position> page=normalizedSearch==null?positions.findAll(pageable):positions.findByNameContainingIgnoreCase(normalizedSearch,pageable);return PageResponse.from(page,PositionMapper::toResponse);}
 @Transactional(readOnly=true) public PositionResponse get(Long id){return PositionMapper.toResponse(entity(id));}
 @Transactional public PositionResponse create(PositionRequest r){String name=InputNormalizer.required(r.name());if(positions.existsByNameIgnoreCase(name))throw new ConflictException("DUPLICATE_POSITION","Position name already exists");Position p=new Position();p.setName(name);p.setDescription(InputNormalizer.optional(r.description()));return PositionMapper.toResponse(positions.save(p));}
 @Transactional public PositionResponse update(Long id,PositionRequest r){String name=InputNormalizer.required(r.name());if(positions.existsByNameIgnoreCaseAndIdNot(name,id))throw new ConflictException("DUPLICATE_POSITION","Position name already exists");Position p=entity(id);p.setName(name);p.setDescription(InputNormalizer.optional(r.description()));return PositionMapper.toResponse(p);}
